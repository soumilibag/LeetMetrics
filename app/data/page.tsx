...
export default function DataDisplay() {
  ...
  // Load the searched data from Session Storage on mount
  useEffect(() => {
    const storedData = sessionStorage.getItem('tracked_leetcode_data');
    if (!storedData) {
      // If someone types /data directly without a valid profile session, kick them back to home
      router.push('/');
    } else {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(result.error || "Failed to sync fresh profile updates.");
      }
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center">
          ...
          <button 
            ...
            disabled={loading}
            className="bg-gray-800 hover:bg-gray-700 border border-gray-700 px-4 py-2 rounded-xl text-xs font-semibold transition disabled:opacity-50"
          >
          ...
          {data.tag_stats ? (
            <div className="space-y-6">
              {['advanced', 'intermediate', 'fundamental'].map((level) => {
                ...
                return (
                  <div key={level}>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">{level} Topics</h4>
